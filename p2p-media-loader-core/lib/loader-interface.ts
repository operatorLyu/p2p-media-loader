...
export type Segment = {                         //定义segment
    readonly id: string;                        //表示只读，只有在创建的时候修改其值
    readonly url: string;
    ...
    data?: ArrayBuffer;                         // ？：表示可选，可以是后面的类型 也可以是undefined
...
export enum Events {
     ...
     * Args: segment
     */
    SegmentLoaded = "segment_loaded",               //加载

    /**
     * Emitted when an error occurred while loading the segment.
     * Args: segment, error
     */
    SegmentError = "segment_error",                 //错误

    /**
     * Emitted for each segment that does not hit into a new segments queue when the load() method is called.
     * Args: segment
     */
    SegmentAbort = "segment_abort",                 //终止

    /**
     * Emitted when a peer is connected.
     * Args: peer
     */
    PeerConnect = "peer_connect",                   //连接

    /**
     * Emitted when a peer is disconnected.
     * Args: peerId
     */
    PeerClose = "peer_close",                       //关闭peer

    /**
     * Emitted when a segment piece has been downloaded.
     * Args: method (can be "http" or "p2p" only), bytes
     */
    PieceBytesDownloaded = "piece_bytes_downloaded",            //下载速率

    /**
     * Emitted when a segment piece has been uploaded.
     * Args: method (can be "p2p" only), bytes
     */
    PieceBytesUploaded = "piece_bytes_uploaded",                //上传速率
}
