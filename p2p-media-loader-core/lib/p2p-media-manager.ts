...
function generatePeerId(): ArrayBuffer {                            //每次新建一个peer都根据WebTorrent产生一个peerID
...
> {
    ...
    private peers = new Map<string, MediaPeer>();   //peers是当前peer的列表，以peerId为索引
    ...
    public constructor(
        ...
        this.peerId = settings.useP2P ? generatePeerId() : new ArrayBuffer(0);//用p2p的话产生peerID
    ...
    };      //设置集群ID，有相同的集群ID才能够建立对等连接

    private createClient = (infoHash: ArrayBuffer): void => {           //创建新的客户端
        if (!this.settings.useP2P) {
            return;             //不适用P2P的话直接返回
        }

        const clientOptions = {             //客户端选项
    ...
    public download = (segment: Segment): boolean => {
        ...
        const peer = candidates[Math.floor(Math.random() * candidates.length)];//随机数乘以候选者的长度向下取整个
        peer.requestSegment(segment.id);         //向候选peer中随机选一个，以段id请求段
        this.peerSegmentRequests.set(segment.id, new PeerSegmentRequest(peer.id, segment));     //将这条请求加入自己的请求map
        return true;
    };
    ...
    private onPeerClose = (peer: MediaPeer) => {
        if (this.peers.get(peer.id) !== peer) {
            ...
            if (index !== -1) {
                peerCandidatesById.splice(index, 1); //删除index位置的一个元素
            }
