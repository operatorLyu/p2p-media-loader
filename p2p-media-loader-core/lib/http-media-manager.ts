...
export class HttpMediaManager extends STEEmitter<"segment-loaded" | "segment-error" | "bytes-downloaded"> {
    ...
    private debug = Debug("p2pml:http-media-manager");                  //大部分变量含义与名称一致

    public constructor(
        readonly settings: {
            httpFailedSegmentTimeout: number;
            httpUseRanges: boolean;
            segmentValidator?: SegmentValidatorCallback;            //segmentUrlBUuilder里由下载片段，p2p还是http下载的以及peer的ID；
    ...
    public download = (segment: Segment, downloadedPieces?: ArrayBuffer[]): void => {
        if (this.isDownloading(segment)) {                  //正在下载就直接返回
            return;
        }

        this.cleanTimedOutFailedSegments();       //清除下载超时失败的segment  
    ...
    private setupXhrEvents = (xhr: XMLHttpRequest, segment: Segment, downloadedPieces?: ArrayBuffer[]) => {
        ...
        xhr.addEventListener("progress", (event) => {
            ...
            this.emit("bytes-downloaded", bytesLoaded);                     //正在下载状态
            prevBytesLoaded = event.loaded;
        });

        xhr.addEventListener("load", async (event) => {
            if (xhr.status < 200 || xhr.status >= 300) {
                this.segmentFailure(segment, event, xhr);           //出错状态
                return;
            }
            ...
            await this.segmentDownloadFinished(segment, data, xhr);             //发送完成
        });

        xhr.addEventListener("error", (event: unknown) => {             //出错状态
            this.segmentFailure(segment, event, xhr);
        });

        xhr.addEventListener("timeout", (event: unknown) => {           //超时状态
            this.segmentFailure(segment, event, xhr);
        });
    };
    ...
    private cleanTimedOutFailedSegments = () => {               //清除超时的非法Segments
        const now = this.now();
    ...
    private now = () => performance.now();                  //获得现在时间
}
